...
export const OutlineButton = ({ title, icon, onClick, onKeyDown, className, disabled }: IButtonProps) => {
    return (
        <button onClick={onClick} disabled={disabled} className={`flex items-center justify-center gap-1 rounded-md p-[0.5rem] font-semibold text-mainColor hover:border-mainColor hover:text-mainColor  h-[38px] text-[14px] ${className} border-[1px] border-mainColor`} onKeyDown={onKeyDown}>
            {icon} {title}
        </button>
    )
}